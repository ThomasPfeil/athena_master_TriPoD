...
Real amp_gas, sigma_gas, xc_gas, dust_diff, rho0_gas, vy_dust, vy_gas;
Real initial_D2G[NDUSTFLUIDS], Stokes_number[NDUSTFLUIDS];
Real amp_dust[NDUSTFLUIDS], sigma_dust[NDUSTFLUIDS], xc_dust[NDUSTFLUIDS];
} // namespace
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  gauss_gas  = pin->GetBoolean("problem", "gauss_gas");
  sigma_gas  = pin->GetOrAddReal("problem", "sigma_gas", 0.0);
  amp_gas    = pin->GetOrAddReal("problem", "amp_gas", 0.0);
  xc_gas     = pin->GetOrAddReal("problem", "xc_gas", 0.0);
  rho0_gas   = pin->GetOrAddReal("problem", "rho0_gas", 1.0);
  vy_gas   = pin->GetOrAddReal("problem", "vy_gas", 1.0);
  ...
  vx0_flag  = pin->GetBoolean("dust", "vx0_flag");
  vy_dust   = pin->GetOrAddReal("dust", "vy_dust", 1.0);
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real den, vel, cs, eps, vx0_dust, depsdx;
  Real x1, x2, x3;
  ...
  for (int k=ks; k<=ke; ++k) {
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=is; i<=ie; ++i) {
        ...
        if(gauss_gas){
          den = rho0_gas + amp_gas * std::exp(-0.5*SQR((x1-xc_gas)/sigma_gas));
        } else {
          den = rho0_gas;
        }
        ...
        phydro->u(IM2,k,j,i) = vy_gas * phydro->u(IDN,k,j,i);
        ...
        if (NDUSTFLUIDS > 0) {
            for(int n=0; n<NDUSTFLUIDS; ++n){
              ...
              int v2_id   = rho_id + 2;
              int v3_id   = rho_id + 3;
              pdustfluids->df_cons(rho_id, k, j, i) = amp_dust[n] * std::exp(-0.5*SQR((x1-xc_dust[n])/sigma_dust[n])) + phydro->u(IDN,k,j,i);
              
              if(vx0_flag){
                eps      = pdustfluids->df_cons(rho_id, k, j, i)/phydro->u(IDN,k,j,i);
                depsdx   = - amp_dust[n]/phydro->u(IDN,k,j,i) * (x1-xc_dust[n])/SQR(sigma_dust[n]) * std::exp(-0.5*SQR((x1-xc_dust[n])/sigma_dust[n]));
                vx0_dust = - dust_diff * depsdx/eps;
              } else {
                vx0_dust = 0.0;
              }

              pdustfluids->df_cons(v1_id,  k, j, i) = vx0_dust * pdustfluids->df_cons(rho_id, k, j, i);
              pdustfluids->df_cons(v2_id,  k, j, i) = vy_dust  * pdustfluids->df_cons(rho_id, k, j, i);
